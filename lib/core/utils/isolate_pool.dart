...
void _isolateEntryPoint(SendPort sendPort) {
  ...
  receivePort.listen((message) async {
    if (message is _FunctionRegistration) {
      ...
      if (function != null) {
        try {
          final result = await Function.apply(function, [message.item]);
...
class _IsolateResponse<R> {
  ...
  /// Creates a copy of this response with a new completer
  _IsolateResponse<R> withNewCompleter() {
    return _IsolateResponse<R>(
      id: id,
      result: result,
...
class _FunctionRegistration<T, R> {
  ...
  _FunctionRegistration({required this.functionId, required this.function});
}
