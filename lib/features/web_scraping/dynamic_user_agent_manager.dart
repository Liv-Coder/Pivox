...
class DynamicUserAgentManager {
  ...
  Future<void> _fetchUserAgentsFromWeb() async {
    try {
      ...
      final response = await http
          .get(Uri.parse('https://www.useragents.me/api'))
      ...
      if (response.statusCode == 200) {
        ...
        if (data['data'] != null && data['data'] is List) {
          ...
          if (fetchedAgents.isNotEmpty) {
            _logger?.info(
              'Fetched ${fetchedAgents.length} user agents from web',
            );
  ...
  String getUserAgentByType(BrowserType type) {
    ...
    _logger?.info(
      'Using ${type.name} user agent: ${_truncateUserAgent(userAgent)}',
    );
    return userAgent;
  }
  ...
  void addSiteSpecificUserAgent(String domain, String userAgent) {
    ...
    if (!_siteSpecificUserAgents[domain]!.contains(userAgent)) {
      ...
      _logger?.info(
        'Added site-specific user agent for $domain: ${_truncateUserAgent(userAgent)}',
      );
    }
  }
