...
class RateLimiter {
  ...
  Future<void> waitForPermission(String domain) async {
    ...
    logger?.info('Rate limit reached for $domain. Waiting for a slot to open up.');
  ...
  void _cleanupOldTimestamps(String domain, DateTime now) {
    ...
    while (_requestTimestampsPerMinute[domain]!.isNotEmpty &&
           now.difference(_requestTimestampsPerMinute[domain]!.first).inMinutes >= 1) {
  ...
  Duration _calculateWaitTime(String domain, DateTime now) {
    ...
    if (minuteCount >= minuteLimit && _requestTimestampsPerMinute[domain]!.isNotEmpty) {
    ...
    if (hourCount >= hourLimit && _requestTimestampsPerHour[domain]!.isNotEmpty) {
